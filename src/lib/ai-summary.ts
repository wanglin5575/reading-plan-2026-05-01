...
export async function enrichArticleWithAi(params: {
  ...
  const maxInput =
    ...
    12000;
  const maxTokens =
    Math.min(parseInt(process.env.AI_SUMMARY_MAX_OUTPUT_TOKENS?.trim() || "800", 10) || 800, 2000) || 800;
  ...
  const hintPub = params.publishedIsoHint?.trim()
    ? `页面元数据推测的发布时间（ISO，可能不准）：${params.publishedIsoHint!.trim()}`
    : "页面元数据未提供可靠发布时间。";
...
${bodyText || "(正文为空)"}`;
  ...
  const systemText = `你是阅读元数据助手。请根据标题与正文节选，输出且仅输出一个 JSON 对象（不要 Markdown、不要代码围栏），字段如下：
